...
(function() {
  ...
  var cKeywords = "abort accept access add all alter and any array arraylen as asc assert assign at attributes audit " +
        ...
        /* CMS */
        "backup break browse bulk cascade clustered coalesce collate compute contains containstable continue convert " +
        "cross current_date current_time current_timestamp current_user dbcc deny disk distributed double dump errlvl " + 
        "except exec execute fillfactor foreign freetext freetexttable full go holdlock checkpoint identity identity_insert " + 
        "identitycol inner join kill left lineno load merge national nocheck nonclustered null nullif offsets opendatasource " + 
        "openquery openrowset openxml outer over percent pivot plan precision print proc raiserror readtext reconfigure " + 
        "replication restore revert right rowcount rowguidcol rule save securityaudit semantickeyphrasetable semanticsimilaritydetailstable " + 
        "semanticsimilaritytable session_user setuser shutdown statistics system_user tablesample textsize top tran transaction " + 
        "try_convert tsequal unpivot updatetext user varying waitfor within group writetext";
        /* CMS */
